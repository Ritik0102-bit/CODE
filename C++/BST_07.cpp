...
int i=0;
int Kth_Largest_BST(TreeNode* Root,int k){
    if(Root==NULL){
        return -1;
    }

    if(Root->right!=NULL){
        int right= Kth_Largest_BST(Root->right,k);
        if(right!=-1){
            return right;
        }
    }
    ...
    if(Root->left!=NULL){
        int left= Kth_Largest_BST(Root->left,k);
        if(left!=-1){
            return left;
        }
    }
...
int main(){
    ...
    int kth_largest = Kth_Largest_BST(Root,2);
